...
########################################################################################                          
    ...
    for ie in range (0,len(par.educ)):
        ...
        for j in range(par.Je1_pos-1, par.Je2_pos+1):### fits only for j = 8
    ...
    # j = Je2+1 : Jc -1 CHECK! IT'S IMPORTANT THAT GRIDS STARTS CHANGING BY EDUCATION ONLY AFTER JE2+1
        ...
        for j in range(par.Je2_pos,par.Jc_pos-1):
    ...
    # j = Jc: Jr
        for j in range (par.Jc_pos-1,par.Jr_pos):
            max_inc         = par.w*exp(inc.age_[ie][0][j-1] + np.max(par.inc_fe[ie][:]) + max(inc_z_val[ie][j-1,:]))
