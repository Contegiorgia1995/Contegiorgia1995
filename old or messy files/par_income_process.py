...
# clear; clc; close all; 
    ...
    N_fe         = par.N_fe = 5
    inc_inno_pos = range(0,N_markov)
    inc_fe_pos   = range(0,N_fe)
    ...
    for ie in range(0,len(EDUC)):
        inc_age_prof[ie][0]      = np.zeros(par.Jr_pos)
        for ia in range (par.Je1_pos-1,par.Jr_pos):
            inc_age_prof[ie][0][ia] = age_prof(ie,age[ia])
        
        inc_age_prof[ie][0][range(par.Je1_pos-1,par.Jr_pos)]     = inc_age_prof[ie][0][range(par.Je1_pos-1,par.Jr_pos)] - inc_age_prof[ie][0][par.Je1_pos-1]


                 
       ## 2. Idiosyncratic shock: eta_e,a,i 
       # Source: NLSY79
    inc_inno_rho    = [0.875153, 0.961399, 0.966375] # autocorrelation of innovation for education groups
    inc_z0_var      = [0.22832, 0.10134, 0.0660616] # Variance of iid shocks to innovation for education groups
    inc_inno_var    = [0.0624742, 0.02341, 0.0291241]   # Variance of iid shocks to innovation for education groups
    if options.AdultRisk == 'N':
       inc_inno_var = 1e-6*inc_inno_var
       inc_z0_var   = 1e-6*inc_z0_var

    inc_z_var       = np.zeros([len(EDUC),len(age)])
    inc_z_var = reshape(inc_z_var,[len(EDUC),len(age)])
    
    for ie in range (0,len(EDUC)):
        rho      = inc_inno_rho[ie]
        z0_var   = inc_z0_var[ie]
        inno_var = inc_inno_var[ie]
        inc_z_var[ie,age_start[ie]] = z0_var
        for ia in range (age_start[ie]+1,par.Jr_pos):
            inc_z_var[ie,ia] = rho**2 * inc_z_var[ie, ia-1] + inno_var
...
######define function to use below (also in a separate file)   
    def discretize_nonstationary_ar_rouwen_P(rho, sigma_z_l,sigma_z_t, n):
  ...
  # It matches persistence of the original process exactly
    ...
    def discretize_nonstationary_ar_rouwen_zgrid(rho, sigma_z_l,sigma_z_t, n):
  #Rouwen method to discretize AR(1), non-stationary, process w symmetric innovations
  # Based on WP Fella, Gallipoli and Pan (2015)
  # It matches persistence of the original process exactly
...
#######
    ...
    for ie in range (0,len(EDUC)): 
        inc_z_val[ie]        = np.zeros([par.Jr_pos,N_markov])
        inc_z_prob[ie]     = np.repeat(np.zeros([par.Jr_pos,N_markov]),N_markov)
        inc_z_prob[ie] = np.reshape(inc_z_prob[ie],[5,34,5])
                                 
        # Initial draw of z
        ia                      = age_start[ie]
        nu                      = sqrt(N_markov-1) * inc_z_var[ie,ia]**0.5
        etagrid                 = np.linspace(-nu, nu, N_markov).T
        inc_z_val[ie][ia,:]     = etagrid
   
        grid_med                 = 0.5*(etagrid[1:]+etagrid[0:-1])
        aux_cdf                  = normcdf(grid_med,0,inc_z_var[ie,ia]**0.5)
        aux_pdf                  = np.append(aux_cdf[0], [aux_cdf[1:]-aux_cdf[0:-1]])
        aux_pdf                  = np.append(aux_pdf, 1-sum(aux_pdf))
        inc_z_prob[ie][:,ia,:] = np.reshape(np.repeat(aux_pdf,N_markov),[5,5])
        
        for ia in range (age_start[ie],par.Jr_pos):
            ############################################################
            inc_z_prob[ie][:,ia,:] =  discretize_nonstationary_ar_rouwen_P(inc_inno_rho[ie],inc_z_var[ie,ia-1]**0.5,inc_z_var[ie,ia]**0.5, N_markov)           
            inc_z_val[ie][ia,:] = discretize_nonstationary_ar_rouwen_zgrid(inc_inno_rho[ie],inc_z_var[ie,ia-1]**0.5,inc_z_var[ie,ia]**0.5, N_markov)
