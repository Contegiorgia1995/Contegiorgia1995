...
# EGM
    ...
    gammac = par.gammac
    beta   = par.beta
    INNO_pos   = range(0,len(innop_prob)) #Because only in last period need to take expectations
    ...
    if sum(indmin)>0:
        ...
        imin = np.argmax(range(0,len(ucp))*(ucp>vmax))
        
        if vmin > np.min(ucp):
            imax = np.min(range(0,len(ucp))*(ucp<vmin)+1e20*(ucp>=vmin))
        elif vmin == min(ucp):
            imax = len(ucp)
        else:
            pass
    ...
    for i in range(int(imin+1),int(imax-1)):
        if i == imin + 1:
            ...
            inno3,Sp3  = np.meshgrid(INNO_pos,Spgrid)# changed
            Vpaux_dense = np.dot(splVp(Sp3[:,0],inno3[0,:]).T,innop_prob)#changed
            # if len(innop_prob)>1:
            #     inno3,Sp3  = np.meshgrid(INNO_pos,Spgrid)
        ...
        for j in range(0,len(c)):
            if c[j]>=0:
                objfunc[j] = c[i]**(1-gammac)/(1-gammac)+ lambdan*n**gamman *c_k[j]**(1-gammac)/(1-gammac) + beta*Vpaux[j]
            else: 
    ...
    #         fprintf('Borrowing constraint - local & potentially binding \n')
            ...
            inno3,Sp3  = np.meshgrid(INNO_pos,Spgrid)#changed
            #Vpaux          = splVp(Sp3,inno3)*innop_prob.T
            Vpaux          = np.dot(splVp(Sp3[:,0],inno3[0,:]).T,innop_prob)#changed
...
###################################################CAREFUL THAT objfunc is defined differently for c-final >00 or c_final <0!!!!!!!!!!!!!!!!!
    ...
    inddecc[0:-2] = np.argmax([inddecc[0:-2],inddecc[2::]],axis =0)
    inddecc[2::] = np.argmax([inddecc[2::],  inddecc[0:-2]],axis = 0)
