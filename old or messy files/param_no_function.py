# -*- coding: utf-8 -*-
"""
Created on Mon Aug  8 09:41:41 2022
...
for ie in range (0,len(par.educ)):
    ...
    for j in range(par.Je1_pos, par.Je2_pos+1):### fits only for j = 8
        ...
        par.grids[ie][j] = np.linspace(int(-borr_limits[j]**(1/curv)),maxE**(1/curv),int(par.Ls[j]))**curv
...
# j = Jc: Jr
    for j in range (par.Jc_pos-1,par.Jr_pos):
        max_inc         = par.w*exp(inc_age_prof[ie][0][j-1] + np.max(par.inc_fe[ie][:]) + max(inc_z_val[ie][j-1,:]))
