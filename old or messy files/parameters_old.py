...
###################################################################
    ...
    par.meanh0          = par_est.meanh0 = 2 ##output of a loop in calibrate file and in par_grid_h0_old
...
########################################################################################                          
    ...
    for i in range (0,len(EDUC)):
        par.inc_fe[i]
    par.inc_fe[0]      = log(H0)                                                #HS Drop
    par.inc_fe[1]      = log(   H0   + par.Reducs[0,0] * H0**par.Reducs[0,1])   #HS Grad
    par.inc_fe[2]      = log(   H0   + par.Reducs[1,0] * H0**par.Reducs[1,1])   #Col Grad
 ...
 40: [[0], [0], [0]],
 41: [[0], [0], [0]]}
   #par.grids = np.array([np.zeros([1,len(par.PHI)]),[np.zeros([1,len(par.PHI)])],[np.zeros([1,len(par.PHI)])]*41,])
   ...
   if options.ParentTrans == 'Endo':
    ...
    for ie in range (0,len(par.educ)):
        ...
        # j = Je1
        par.grids[par.Je1_pos-1][ie] = par.PHI ##### par.phi is a sequence
        #j = Je2
        maxE = par.PHI[-1]*2;
        ...
        for j in range(par.Je1_pos, par.Je2_pos+1):### fits only for j = 8
            ...
            par.grids[j][ie] = np.linspace(int(-borr_limits[j]**(1/curv)),maxE**(1/curv),int(par.Ls[j]))**curv
    ...
    # j = Je2+1 : Jc -1 CHECK! IT'S IMPORTANT THAT GRIDS STARTS CHANGING BY EDUCATION ONLY AFTER JE2+1
        ...
        for j in range(par.Je2_pos+1,par.Jc_pos-1):
            ...
            max_inc         = par.w*exp(inc_age_prof[ie][0][j-1] + np.max(par.inc_fe[ie][:]) + np.max(inc_z_val[ie][j-1,:]))*2 # par.inc.age_prof to be changed in par_income_process to get rid of [0] as it is defined wrong and we should not need [0]
 ########################################################################################################          
            maxS            = max(par.grids[ie][j-1](end)*1.3,max(max_inc,maxWY))
            par.grids[j][ie] = np.concatenate(par.grids[j][ie],(np.linspace(int(-borr_limits(j)**(1/curv)),int(maxS**(1/curv),par.Ls[j]))**curv)
    
    # j = Jc: Jr
        for j in range (par.Jc_pos,1:par.Jr_pos):
            max_inc         = par.w*exp(par.inc.age_prof{ie,1}(j-1) + np.max(par.inc_fe[ie]) + max(inc_z_val[ie][j-1,:]))
            maxS            = max(par.grids{ie,j-1}(end)*1.3,max_inc*3);
            par.grids{ie,j} = np.linspace(-borr_limits[j]**(1/curv),maxS**(1/curv),par.Ls[j])**curv

    # j = Jr+1 : Jd
        maxR                = par.grids{ie,j}(end)*2
        for j=par.Jr_pos+1:par.Jd_pos
            par.grids{ie,j}    = np.linspace(-borr_limits[j]**(1/curv),maxR**(1/curv),par.Ls[j])**curv
######################################################################################################
...
## Fertility Policies
    ...
    if options.init_transfer == 'N':
        ...
        par.init_trans = np.reshape(par.init_trans,[3,41])
...
###########################################################################################     missing options.init_trans_size
        ...
        par.init_trans = np.zeros([len(par.educ),par.Jd_pos])
        par.init_trans = np.reshape(par.init_trans,[3,41])
        par.init_trans[2,par.Je2_pos-1:par.Je2_pos]    = 2*options.init_trans_size*par.p_model_data
############################################################################################
