...
for educ_p = aux_educ_p(ind_educ_p)
    ...
    for is_par = aux_inc(ind_inc)
        ...
        mu_int_vec               = Q' * mu0int_vec;
        clear mu0int_vec
        mu_int2(educ_p,is_par,:,:,:,:)  = reshape(mu_int_vec,1,1,length(S),length(H),length(PSY),length(EDUC));
        clear mu_int_vec
    end
end
% Remove Psychic cost
...
mu_chetty1 = zeros(length(EDUC),length(Inc_par_grid),length(Spchild),length(Hpchild),length(EDUC)); 
...
for j_c_pos  = par.Je1_pos+1:par.Jc_pos
    ...
    if j_pos_c-1 <= par.Je1_pos + 1% First year of education
        ...
        Spol(:,:,2)   = squeeze(Se_pol_c{2,j_pos_c-par.Je1_pos}(:,:,1));
        Spol(:,:,3)   = squeeze(Se_pol_c{3,j_pos_c-par.Je1_pos}(:,:,1));
    ...
    prob_educ_p = squeeze(sum(sum(sum(sum(mu_chetty(:,:,:,:,:),5),4),3),2));
    ind_educ_p  = (prob_educ_p>0);
    aux_educ    = 1:length(EDUC);
    for educ_p  = aux_educ(ind_educ_p)
        prob_inc = squeeze(sum(sum(sum(mu_chetty(educ_p,:,:,:,:),5),4),3));
        ind_inc  = (prob_inc>0);
        aux_inc  = 1:length(Inc_par_grid);
        for is_par = aux_inc(ind_inc)
            mu0int_aux                  = squeeze(mu_chetty(educ_p,is_par,:,:,:));
            mu0int_vec                  = mu0int_aux(:);
            clear mu0int_aux
            mu_int_vec                  = Q' * mu0int_vec;
            clear mu0int_vec
            mu_chetty1(educ_p,is_par,:,:,:)  = reshape(mu_int_vec,1,length(Spchild),length(Hpchild),length(EDUC));
        end
    end
