function [V,C,Sp] = prob_work_old (par,options,Vp,Cp,j_pos)
% Solve household problem when old; at j = Jt has to transfer OAS
...
for educ = 1:length(EDUC)
    ...
    for ife=1:length(FE_pos)
        ...
        for inno = 1:length(INNO_pos)
            ...
            dispinc = w*exp(AGE_PROF{educ,1}(j_pos)) * exp(FE(ife)) * exp(INNO(inno))*(1-lambda);
