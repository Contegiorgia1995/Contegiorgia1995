function C = ChildCost(par,labor_inc,n,T,options) %#ok<INUSD>
...
% l_ret               = 0.4757;
% l_ret               = 0.7;
% switch options.ChildCost
%     case 'OppC_Const'
% %         C           = T *       (n*par.fam_size)^l_ret * (labor_inc* lost_wnanny   + (lost_total-lost_wnanny) * par.pn ) ...
% %                       + (1-T) * (n*par.fam_size)^l_ret * (labor_inc* lost_total);
% %             If estimating lost_total, lost_wnanny, l_ret: use below
%           C           = T *       n^l_ret * (labor_inc* lost_wnanny   + (lost_total-lost_wnanny) * par.pn ) ...
%                       + (1-T) * n^l_ret * (labor_inc* lost_total);
%     case 'Constant'
