function [mup,Q_ergo_out] = trans_work(par,mu0,Spol,j_pos,Q_ergo_in)
% keyboard
...
%% 1. Choice of savings 
% mu0int                    = zeros(length(S),length(H),length(EDUC));
mu0int_vec    = mu0(:);
...
mup        = reshape(mu_int_vec,length(Sp),length(FE_pos),length(EDUC),length(INNO_pos));
end