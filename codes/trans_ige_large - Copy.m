function [mu_chetty] = trans_ige_large(par,mu_ige0,tau0,S_pol_c,Se_pol_c,options)
% Intergenerational distributions
% keyboard
switch options.timer_on
    case {'Y'}
        fprintf('\n Intergenerational distributions Large\n \n');
end
...
% Distributions
mu0int        = zeros(length(EDUC),length(Inc_par_grid),length(S),length(H),length(PSY),length(EDUC));
mu_int2       = zeros(length(EDUC),length(Inc_par_grid),length(S),length(H),length(PSY),length(EDUC));

prob_educ_p = squeeze(sum(sum(sum(sum(mu_chetty_int,5),4),3),2));
ind_educ_p  = (prob_educ_p>0);
aux_educ_p  = 1:length(EDUC);
for educ_p = aux_educ_p(ind_educ_p)
    prob_inc = squeeze(sum(sum(sum(mu_chetty_int(educ_p,:,:,:,:),5),4),3));
    ind_inc  = (prob_inc>0);
    aux_inc  = 1:length(Inc_par_grid);
    for is_par = aux_inc(ind_inc)
        mu0int_aux          = squeeze(mu0int(educ_p,is_par,:,:,:,:));
        mu0int_aux(:,:,:,1) = mu_chetty_int(educ_p,is_par,:,:,:);
        mu0int_vec          = mu0int_aux(:);
        clear mu0int_aux
        % new distribution:
        mu_int_vec               = Q' * mu0int_vec;
        clear mu0int_vec
        mu_int2(educ_p,is_par,:,:,:,:)  = reshape(mu_int_vec,1,length(S),length(H),length(PSY),length(EDUC));
        clear mu_int_vec
    end
end
...
mu_chetty1 = zeros(length(EDUC),length(Inc_par_grid),length(Spchild),length(Hpchild),length(PSY),length(EDUC)); 
...
for j_c_pos  = par.Je1_pos+1:par.Jc_pos
    ...
    prob_psy = squeeze(sum(sum(sum(sum(sum(mu_chetty,6),4),3),2),1));
    ind_psy  = (prob_psy>0);
    ...
    for ipsy  = aux_psy(ind_psy)
        ...
        prob_educ_p = squeeze(sum(sum(sum(sum(mu_chetty(:,:,:,:,ipsy,:),6),4),3),2));
        ind_educ_p  = (prob_educ_p>0);
        aux_educ    = 1:length(EDUC);
        for educ_p  = aux_educ(ind_educ_p)
            prob_inc = squeeze(sum(sum(sum(mu_chetty(educ_p,:,:,:,ipsy,:),6),4),3));
            ind_inc  = (prob_inc>0);
            aux_inc  = 1:length(Inc_par_grid);
            for is_par = aux_inc(ind_inc)
                mu0int_aux                  = squeeze(mu_chetty(educ_p,is_par,:,:,ipsy,:));
                mu0int_vec                  = mu0int_aux(:);
                clear mu0int_aux
                mu_int_vec                  = Q' * mu0int_vec;
                clear mu0int_vec
                mu_chetty1(educ_p,is_par,:,:,ipsy,:)  = reshape(mu_int_vec,1,length(Spchild),length(Hpchild),1,length(EDUC));
            end      
        end
    end
...
clear mu_chetty1
% Remove Psychic cost
mu_chetty   = squeeze(sum(mu_chetty,5));

switch options.timer_on
    case {'Y'}
        fprintf('ige large, time: %3.1f sec \n',toc)
end
