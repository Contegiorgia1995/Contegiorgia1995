function [ c_final,sp_final,boundgrid ] = GEGM( par,ucp, dispinc,Spgrid,S,splVp,innop_prob )
% EGM
...
for i=imin+1:imax-1
   ...
   c = (1+r_sav).*a_candidate.*(a_candidate>=0) + (1+r_debt).*a_candidate.*(a_candidate<0)  +dispinc - Spgrid';
...
%       c_opt(i)  = nan;
      ...
      c = (1+r_sav).*a_candidate.*(a_candidate>=0) + (1+r_debt).*a_candidate.*(a_candidate<0)  +dispinc - Spgrid_dense';
...
%         fprintf('Borrowing constraint - local & potentially binding \n')
        ...
        c = repmat((1+r_sav) .* S_bc .* (S_bc >= 0) + (1+r_debt) .* S_bc .* (S_bc < 0),size(Spgrid')) +dispinc - repmat(Spgrid',size(S_bc));
...
for i = grid_prob(inddecc)
    ...
    c = (1+r_sav).*S(i).*(S(i)>=0) + (1+r_debt).*S(i).*(S(i)<0)  +dispinc - Spgrid_dense';
